    ...
    public void onPlace(final BlockPlaceEvent e) {
        if (e.getBlock().getType() == Material.TORCH || e.getBlock().getType() == Material.WALL_TORCH) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.plugin, (Runnable)new Runnable() {
                @Override
                public void run() {
                    if (Main.plugin.getConfig().getString("TorchLocations." + location.getBlockX() + location.getBlockY() + location.getBlockZ()) != null && (e.getBlock().getType() == Material.TORCH || e.getBlock().getType() == Material.WALL_TORCH)) {
