    ...
    public void onPlace(final BlockPlaceEvent e) {
        if (e.getBlock().getType() == Material.SOUL_CAMPFIRE) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.plugin, (Runnable)new Runnable() {
                @Override
                public void run() {
                    if (Main.plugin.getConfig().getString("CampfireLocations." + location.getBlockX() + location.getBlockY() + location.getBlockZ()) != null && e.getBlock().getType() == Material.SOUL_CAMPFIRE) {
